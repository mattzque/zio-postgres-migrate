...
case class MigrationCollection(migrations: List[Migration]):
  ...
  def toTableString(title: String): String =
    ...
    def formatTable[T](values: List[T], columns: List[String], widths: List[Int], row: T => List[String]): String =
      val delim = " | "
      val padding = ' '
      columns.zipWithIndex.map((v, index) => v.padTo(widths(index), padding)).mkString(delim)
        ...
        + "\n"
        + values.map(
        value => row(value).zipWithIndex.map((v, index) => v.padTo(widths(index), padding)).mkString(delim)
      ).mkString("\n")
